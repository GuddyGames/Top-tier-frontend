...
export default function Wallet() {
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-8 pt-5 sm:px-8">
    ...
    <div className="mt-3 space-y-2">{activities.length?activities.map((x,i)=><div key={x.id||i} className="tt-card flex items-center justify-between rounded-2xl p-3"><div><p className="text-xs font-semibold">{x.action||x.type||'Activity'}</p><p className="text-[10px] text-ink-muted">{x.created_at?new Date(x.created_at).toLocaleString():'Recent'}</p></div><span className="text-xs font-bold text-gain">+{x.points||x.amount||0} pts</span></div>):<div className="tt-card rounded-2xl p-6 text-center text-xs text-ink-muted">No wallet transactions yet.</div>}</div>
  </div>;
}