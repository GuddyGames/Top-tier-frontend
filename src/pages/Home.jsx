...
export default function Home({ goToTerminal, goToLearn }) {
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-8 pt-5 sm:px-8">
    ...
    <div className="mt-5 flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full border-2 border-brand-cyan/50 bg-brand-blue/20 text-xl">👤</div><div><p className="text-[10px] text-ink-muted">Good morning,</p><h1 className="font-display text-base font-bold text-white">{name}</h1><p className="text-[10px] text-brand-cyan">★ Level {Math.max(1,Math.floor((s.total_points||0)/500)+1)} · {(s.total_points||0).toLocaleString()} pts</p></div></div>
